    ...
    return (
        <>
            ...
            <div className="fixed top-4 right-4 z-[999] flex flex-col gap-3 pointer-events-none" style={{ maxWidth: '380px' }}>
                {toasts.map((toast) => {
                    ...
                    return (
                        <div
                            key={toast.id}
                            className="pointer-events-auto bg-card border border-violet-500/30 shadow-2xl shadow-violet-500/10 rounded-2xl overflow-hidden animate-in slide-in-from-right-8 fade-in duration-500 w-[360px] max-w-full"
                        >
                            {/* Accent top bar */}
                            <div className="h-1 bg-gradient-to-r from-violet-500 to-purple-500 w-full" />
                            <div className="p-4 flex gap-3">
                                <div className="shrink-0 p-2 bg-violet-500/10 text-violet-500 rounded-lg h-fit">
                                    <Megaphone className="w-4 h-4" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start gap-2">
                                        <span className="text-xs font-bold text-violet-500 uppercase tracking-wide">Admin Broadcast</span>
                                        <button
                                    ...
                                    {url && (
                                        <a href={url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-violet-500 hover:text-violet-600 bg-violet-500/10 px-2 py-1 rounded-md transition-colors">
                                    ...
                                    <p className="text-[10px] text-muted-foreground mt-2">
                                        ...
                                        {" · "}
                                        <button onClick={handleOpenDrawer} className="text-violet-500 hover:underline font-semibold">View All</button>
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div className={cn(
                ...
                {/* Drawer Header */}
                <div className="px-6 py-5 border-b border-border flex justify-between items-center bg-gradient-to-r from-violet-500/5 to-purple-500/5">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-violet-500/10 text-violet-500 rounded-xl shadow-sm">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-muted/5">
                    {announcements.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-60 py-20">
                            <div className="p-5 bg-violet-500/10 rounded-2xl mb-5">
                                    ...
                                    <div className="flex self-start max-w-[92%] sm:max-w-[88%] animate-in fade-in slide-in-from-left-4 duration-300">
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-2 mb-1.5 ml-1">
                                                <span className="font-bold text-xs text-violet-500 bg-violet-500/10 px-2 py-0.5 rounded-md border border-violet-500/20">Admin Ops</span>
